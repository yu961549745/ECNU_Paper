...
\begin{equation}
    \bbrace{a_0=\alpha,k=\pm\frac{\sqrt{\beta}}{2},c=-k\frac{\alpha \beta+1}{2}}.
\end{equation}

当$\gamma=-1,\delta=2$时, $m=2$, 原方程有解 
\begin{equation}
\bbrace{
c=\frac{ 16\,{k}^{4}\,{{a}_{2}}-12\,\alpha\,\beta\,{k}^{2}-{\beta}^{2}\,{{a}_{2}}-12\,{k}^{2} }{12\,k},
{{a}_{0}}=\frac{ 12\,\alpha\,{k}^{2}-8\,{k}^{2}\,{{a}_{2}}+\beta\,{{a}_{2}} }{12\,{k}^{2}},
{{a}_{1}}=0
} .
\end{equation}
这和上一个例子一样, 也是一个可以任意阶平衡的方程.
}
\end{example}
...
\begin{equation}
    {{u}_{t}}+{{u}_{x}}+\alpha\,{{u}_{xxx}}+\beta\,u\,{{u}_{x}}+\gamma\,u\,{{u}_{xxx}}+\delta\,{{u}_{x}}\,{{u}_{xx}}=0.
\end{equation}
当$\gamma=-1,\delta=3$时, $m=1$, PExpand 可以得到 
\begin{equation}
