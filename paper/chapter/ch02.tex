...
\end{equation}
这个方程在\reftab{verify}中被称为是(4+1)维 Fokas-T方程. 将原方程降为(3+1)维 维方程之后, 对应的TPE变为
\begin{equation}
...
\end{split}
\end{equation}
从\reftab{verify}中可以看出, 当$\PS=\{1,2,3\}$时, 方程无\TrueSol{}. 但是当$\PS=\{1,2\}$或$\PS=\{1,3\}$时, 方程有\TrueSol{}. 事实上, 和大多数(3+1)维 维方程一样, 当$(p_i,q_i)~(i=1,2,\cdots)$线性相关时, \refeqnn{Fokas-T}有\TrueSol{}.
...
\end{equation}
该方程在\reftab{verify}中被记作(4+1)维 Fokas-T-2. 该方程的关键参数是
\begin{equation}
\begin{split}
