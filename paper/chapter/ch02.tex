...
在\refalg{rpsolve}中, 我们定义了\cd{rpsolve}函数. 在该函数中, 我们用$s$表示已经求解的变量的解. 当$s$是一个集合时, 表示含有多个解, 将遍历每个解进行代入求解; 当$s$是一个列表时, 表示这是一个解, 将代入这个解进行求解. 调用\cd{rpsolve(1,{},eqs,vrs)}就能求得所有解. 此外, 因为求解过程中即需要求解代数方程, 也可能求解微分方程, 所以我们基于 Maple 中的 \cd{PDEtools:-Solve} 进行求解. 
