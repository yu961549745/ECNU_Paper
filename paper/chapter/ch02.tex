...
\item 返回值\texttt{sh}是一个\texttt{SolHolder}对象. 它保存了计算的中间结果, 如$\omega,h_{i,j},\theta,b_{i,j}$等. 这些参数能够用于构造孤子解\D 呼吸子解和lump解. 
\end{compactitem}
...
\reftab{verify}中所有例子的运行时间如\reftab{runtime}所示. 从\reftab{runtime}中可以看出, method-1是最快的. 这是因为 method-1 在验证2-呼吸子解时存在显著优势, 而2-呼吸子的验证耗时又是最长的. 因此, \texttt{sh:-verify\_sol}的默认选项是\texttt{method=1}. 
