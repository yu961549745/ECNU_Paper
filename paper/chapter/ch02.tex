...
此外, 因为求解过程中既需要求解代数方程, 也可能求解微分方程, 故采用 Maple 中的 \cd{PDEtools:-Solve} 函数进行求解. \red{因为\refalg{rpsolve}是一个涉及求解的算法, 它的迭代步数受解的数量影响, 所以讨论其复杂度是没有意义的.} 
...
\end{split}
\end{equation}
事实上, $N_{new}(m)$是 OEIS A066223 的第$m$项\footnote{OEIS A066223: \url{https://oeis.org/A066223}}, 它的增长要比阶乘更快. \red{\refalg{lump-gen}的复杂度正比于$m$-lump解所包含的项数, 即$\OC\sbrace{N_{new}(m)}$. 采用\refeqn{f-lump-old}生成 $m$-lump 解, 则式中的加法项个数为
\begin{equation}
