...
事实上, $\ell_1$-优化能够转化为一个线性规划\citep{L1_regression}, 即
\begin{equation}
\begin{split}
...
现有的ELP算法有: Maple中的\cd{simplex}\D Mathematica 中的\cd{LinearProgramming}\D \spell{SoPlex}\citep{soplex}和\spell{Qsopt-ex}\citep{qsoptex}等. 并且, 这些 ELP 算法都是针对有理数域上的问题进行求解的, 因此需要将我们的问题转化到有理数域上. 
...
尽管 ILP 要比 LP 更难, 但是数值计算的优势使得 NILP 算法比 ELP 算法更快. 例如, 在问题规模小于250时, \cd{NILP} 要比 \cd{SLP} 和 \cd{mmaSLP}更快. 而\cd{MatlabILP}则始终显著地快于\cd{SLP} 和 \cd{mmaSLP}. 从图中也可以看出, Maltab 的 ILP 算法要比 Maple 的优秀许多. 这是因为 Matlab 在分支定界法\citep{wolsey1998integer}的基础上, 还集成了许多 LP 和 ILP 中的预处理技术\citep{preLP1995,preLP2003,preMILP1994,cut2008}, 同时还引入了许多优秀的启发式求解策略\cite{Heuristics2005,berthold2006primal}. 事实上, 除了 Matlab 以外, Gurobi\cite{gurobi} 也是一个非常优秀的 ILP 求解器, 且它在一些复杂问题上性能要优于 Matlab. 但是, Gurobi 在本文的问题上性能优势不大, 为了简化本文软件包的依赖环境, 我们没有集成 Gurobi. 
