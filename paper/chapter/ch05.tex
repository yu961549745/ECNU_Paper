...
\section{问题的提出}
在非线性微分方程求解的过程中往往需要进行非线性积分表达式的化简. 因此, 本文将非线性积分化简作为一个具有挑战性的问题进行研究.
...
\item 基于\refalg{FindAllRules}\D \refalg{FindRuleForPair}\D \refalg{FindRuleForVars1} 和\refalg{FindRuleForVars2}, 找到所有的二项合并规则.
...
综上所述, 在最差情况下, 我们的算法关于阶数的时间复杂度是指数的. 在有限的阶数条件下, \red{该算法的复杂度关于输入表达式的次数是多项式的}. 在阶数和次数都有限的情况下, 该算法的复杂度关于输入规模是线性的. 因此, 我们的算法在实际应用过程中往往是高效的.  
