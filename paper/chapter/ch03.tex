...
一个方程组的复杂度是由三个个方面决定的: 一方面是方程的数量, 方程的数量越多则复杂度越高; 另一方面是方程组中变量的个数, 变量的个数越多则复杂度越高; 还有一方面是方程组中表达式的复杂度, 单个表达式长度越长, 次数越高, 则这个表达式越复杂. 出于这些考虑, 按复杂度排序的逻辑如下:
\begin{compactenum}[(1)]
...
在微分方程的求解过程中, 许多方法最终都将归结为非线性代数方程组的求解, 如齐次平衡方法\D 双曲正切方法和 Jacobi椭圆函数方法等, 最终都需要求解非线性代数方程组. 在\refchp{ch02}中, 我们基于简单 Hirota 方法求得了 NLEE 的孤子解\D 呼吸子解和lump 解. 其实, 也可通过直接代数方法来求解它们. 相比而言, \refchp{ch02}中的算法求解方法效率更高. 但是, 对于孤子解和lump解的相互作用解, 目前只能利用直接代数方法进行求解. 该求解方法中往往会产生大规模的非线性代数方程组, 我们就以它为例来展示 PGSolve 的有效性和高效性. 
...
将\refeqn{f-ns1l}代入到\refeqnn{feq-ns1l}中, 提取指数函数和各个自变量的系数, 就能得到关于\refeqn{vrs-ns1l}中所有参数的代数方程组. 求解该代数方程组就能获得原方程的 nS-1L 解. 
...
sh:=gen_eq(eq,0,p,q);
\end{verbatim}
这里, \cd{gen\_eq} 的第一个参数是一个NLEE, 第二个参数\cd{n}表示相互作用解中孤子解的阶数. 而\cd{p,q}则对应于\refeqn{f-ns1l}中的$p,q$, 用于给待定参数设置变量名. 此外, 和 TwSolver 一样, NS1L 的求解也过程中也涉及到\Painleve{}展开法的相关计算, 故也存在多解的情况. 所以, \cd{gen\_eq}指定了\cd{select\_solution}参数来供人工选择 TPE 的解. 最后, 返回值\cd{sh}是一个\cd{EqnHolder}对象. 该对象的作用类似于TwSolver中的\cd{SolHolder}对象, 下面将一一展示它的作用. 
...
在\reffig{sb0}中, 0号解是空集, 此时剩余35个方程. 第一轮求解之后, 生成了编号为1,2的两个解. 此时, 未解的方程只有5个. 也就是说, 虽然我们在求解时只求解了5个方程, 但是解得的解满足了30个方程. 这正是 PGSolve 效率高于传统求解算法的主要原因. 在第二轮求解时, 我们发现2号解不是原方程的解, 而1号解则生成了3个原方程的解. 
...
\mk{因为在上述过程我们基于直接代数方法获得了 lump 解}{上面一句已经说过流程了, 这句的转折注重的是结果}, 而 TwSolver 也能求 lump 解, 于是我们就来比较两种方式得到的解有什么不同. 

由\reftab{verify}可知, (3+1)维 YTSF方程在$\PS=\bbrace{1,3}$时能通过 TwSolver 获得 lump 解. 调用\todo{代码后面加什么标点很尴尬, 按照代码的语法应该加分号, 加逗号虽然组成了句子但是有歧义}
\begin{verbatim}
...
\right\}
\end{equation}
时, \refeqn{0S1L-1}所对应的解能转化为\refeqn{0S1L-3}. 也就是说, 直接代数方法得到的 lump 解比简单 Hirota 方法得到的解更具一般性. 

\subsection{直接求解与继承求解}

接下来, 我们将求解(3+1)维 YTSF方程的 1S-1L 解. 按照之前的步骤, 我们调用如下\red{命令}进行求解. 
\begin{verbatim}
sh:=gen_eq(eq,1,p,q);
...
\subfigure[直接求解的分支图 \label{sb1-d}]{
    \includegraphics[width=.4\textwidth]{fig/1S1L-dir.pdf}
}
\hspace{2cm}
\subfigure[继承求解的分支图 \label{sb1-e}]{
    \includegraphics[width=.28\textwidth]{fig/1S1L-ext.pdf}
}
...
经验证发现, \refeqn{0S1L-1}对应的解和\citett{zhangYTSF}中的解是等价的. \red{同时, NS1L也获得了新的解.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.88\textwidth]{fig/2S1L-ext.pdf}
...
经验证, 我们发现\refeqn{2S1L-eq}和\refeqn{2S1L-plot}是等价的. 所以, 直接求解比继承求解多获得两个解, 且直接求解的结果形式更复杂\D 用时更长. 所以, 在对解的完整性要求不严格时, 继承求解是更优的选择.
