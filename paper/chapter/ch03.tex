...
按照上述策略进行排序就能保证所求解的前$n$个方程构成的方程组是最简的. 接着, 我们就要尽可能地控制解的数量以减少分支的数量. 在变量数目不变的情况下, 一个方程组解的数量和方程的数量相关. 一般而言, 方程数量越多, 约束越苛刻, 则解的数量就越少. 但是方程数量越多, 则求解这个方程组所需要的时间越长. 因此, 求解方程的数量是一个需要权衡的数字, 可以由用户根据实际情况进行设置, 而本文一般取 $n=5$.
...
将\refeqn{f-ns1l}代入到\refeqnn{feq-ns1l}中, \red{提取指数函数系数之后, 令各个自变量的系数为零}, 就能得到关于\refeqn{vrs-ns1l}中所有参数的非线性代数方程组. 求解该方程组就能获得原方程的 nS-1L 解.
...
从上述过程可知, 我们基于直接代数方法也可推导出lump解, 而 TwSolver 也能求 lump 解, 于是我们就来比较两种方式得到的解有什么不同. 
...
经验证发现, \refeqn{0S1L-1}对应的解和\citett{zhangYTSF}中的解是等价的. 同时, NS1L也获得了新的解.
