...
\end{split}
\end{equation}
则\texttt{NS1L}得到的三个解为 \todo{先去集成一下 Maple 的 LaTeX 工具函数再来}
% \begin{eqnarray}
    
% \end{eqnarray}

取\texttt{k=1..3}, 采用相同的参数进行绘制,
\begin{verbatim}
sh:-plot_sol(
    sols[k],
    [x=-30..30,y=-30..30,alpha=1,t=0,z=0,p[1,2]=2,q[2]=1/3],
    rest_assign,
    grid=[100,100],orientation=[-78,60,-3]
);
\end{verbatim}
可以得到如\reffig{fig-1S1L}所示的三个解. 
