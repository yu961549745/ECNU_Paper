...
\label{ceq}
\end{equation}
其中 $f(x)$ 是未知函数, $g(x)$表示任意多项式, $c_k (k=0,\cdots,l)$是任意常数. 该方程的求解在很久以前就已经被解决了. 我们可以简单地将\refeqnn{ceq}拓展为线性多项式系数的方程
\begin{equation}
...
\section{本文的选题和主要工作}
随着计算机代数系统的不断发展, 衍生了许多构造非线性系统精确解的机械化方法. 本文在前人工作的基础上, 针对非线性系统精确解的相关机械化算法进行研究, 开展了如\reffig{outline}所示的一些工作. 在\reffig{outline}中, 椭圆表示方法\D 矩形表示软件包. 其中 矩形内部的第一行是软件包的名称, 第二行是软件包用途的简要说明. \red{这些软件包可以分为 4 类: 黄色的NEM\D PExpand 和 PGSolve 是基础工具; 蓝色的 NLREPS 用于求解差分方程; 绿色的 TwSolver 和 NS1L 用于求解微分方程; 粉色的 IntSimplify 用于进行积分化简.}
...
在\refchp{ch02}中, 因为由 Hirota 方法推导出的$n$孤子解公式对可积方程成立, 对不可积方程往往不成立, 本文引入了一种参数约束条件, 将$n$孤子解的公式推广到不可积方程的情形. 在此基础上基于\Painleve{}展开法\D 简单Hirota 方法\D 共轭参数法和长极限法, 发展出了构造非线性演化方程的孤子解\D 呼吸子解和 lump 解的机械化算法, 研发了相应的软件包 TwSolver, 并在编程实现时对上述方法的一些细节进行了优化. 
